...
namespace {
    ...
    void nullary(const Op op, bool &rad_on) {
        switch (op) {
            case Op::RAD:
                rad_on = true;
                return;
    ...
    {
        double trig_current = current;
        double atrig_coefficient = 1;
        if (!rad_on) {
            trig_current = current * M_PI / 180;
            atrig_coefficient = 180 / M_PI;
        }
        switch (op) {
            case Op::NEG:
                return -current;
            case Op::SIN:
                return std::sin(trig_current);
            case Op::COS:
                return std::cos(trig_current);
            case Op::TAN:
                return std::tan(trig_current);
            case Op::CTN:
                return 1 / std::tan(trig_current);
            case Op::ASIN:
                if (current < -1 || current > 1) {
                    std::cerr << "Bad argument for asin: " << current << std::endl;
                    return current;
                }
                return std::asin(current)* atrig_coefficient;
            case Op::ACOS:
                if (current < -1 || current > 1) {
                    std::cerr << "Bad argument for acos: " << current << std::endl;
                    return current;
                }
                return std::acos(current) * atrig_coefficient;
            case Op::ATAN:
                return std::atan(current) * atrig_coefficient;
            case Op::ACTN:
                return (M_PI / 2 - std::atan(current)) * atrig_coefficient;
            case Op::SQRT:
                if (current > 0) {
    ...
    double binary(const Op op, const double left, const double right)
    {
        switch (op) {
            ...
            case Op::POW:
                if (right == 0 && left == 0) {
                    std::cerr << "Bad right argument for power (0^0)" << std::endl;
                    return 0;
                }
                else {
...
double process_line(const double current, bool& rad_on, const std::string & line)
{
    ...
    switch (arity(op)) {
        ...
        case 1: return unary(current, op, rad_on);
        case 0: nullary(op, rad_on);
        default: break;
    }
    return current;
}
